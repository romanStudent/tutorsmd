...
export const ChangePasswordForm = ({ handleSubmit, onSubmit, register, errors, isLoading, serverError }: any) => {
  const { t } = useTranslation('settings');

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <h2 className="text-lg font-semibold text-slate-900">{t('changePassword.title')}</h2>
      ...
      <Field label={t('changePassword.oldPassword')} error={errors.oldPassword?.message} htmlFor="oldPassword">
        <input
          ...
          className={inp(!!errors.oldPassword)}
