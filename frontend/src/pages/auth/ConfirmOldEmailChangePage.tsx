...
import { useConfirmOldEmailChangeMutation } from '@shared/api/authApi';
import { Layout } from '@widgets/layout/index';

export default function ConfirmOldEmailChangePage() {
  const { token } = useParams<{ token: string }>();
  const [confirm] = useConfirmOldEmailChangeMutation();
        ...
        <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200
          ...
          {status === 'success' && (
            <>
              ...
              <h1 className="text-xl font-bold text-slate-900">Bestätigt</h1>
